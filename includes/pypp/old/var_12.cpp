...
class var {
public:
	union {//all kinds of data
		struct {//for non-const ref=obj& or ptr=&obj, save its pointer	(for const, will add in the future)
			void * p;//4 bytes
			struct {
				bool inheap:1;	//record if the target obj is in heap or not, true if in heap
				bool super:1;	//if var is the only one holding the pointer to a obj in heap, 
							//it is the obj's super owner, called a super var;
							//when the super var change its type, it will delete the p directly;
							//when another var copy construct from a superowner var, the super var should
							//first be set to non-super, give the ptr to heap memory manager,
							//then copy its data area to another var, both of the two vars are non-super
				bool borrowed:1;
				bool lend:1;
			} attr;//hold attributives
			struct {
				other;
			}
		} pointer;
	...
	const void*& p;//it always ref to p or this
				//when var is holding a pointer, it is p
				//when var is boxing a built-in type, it is this
				//problem: how to change it when var change
...
private:
	union {//data of all kinds of var, only one kind at a time
		...
		struct {//for non-const ref=obj& or ptr=&obj, created in var member functions save its heapobj pointer
			void * p;//4 bytes
