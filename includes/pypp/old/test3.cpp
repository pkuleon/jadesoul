...
public:
	heapobj() { malloc(); }
	~heapobj() { free(); }
	T* get() {//get a ptr to the obj, be careful, do not delete p outside
		if (p==0) return 0;
		inc();
	}
	void del() {//release a ptr to the obj
		if (p==0) return;
		dec();
	}
...
private://data
	...
	int refcnt;//the refernce count to this object, equals to how many times calling get() without a del()
};
...
public:
	typedef heapobj<py::str> str;
	typedef heapobj<py::list> list;
	typedef heapobj<py::tuple> tuple;
	typedef heapobj<py::set> set;
	typedef heapobj<py::dict> dict;
