# 1 "pypp-unit-tests.cpp"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "pypp-unit-tests.cpp"
# 15 "pypp-unit-tests.cpp"
# 1 "pypp.hpp" 1
# 34 "pypp.hpp"
class memmgr;
class var;
...
# 367 "pypp.hpp"
 ...
 void init_as_dict(var* begin=0, var* end=0) {
  ...
  } else {cout<<"trace:\n\t"<<"in init_as_dict: you should give me even numbers of vars"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<455<<endl;assert(false);};};
 };
 ...
 var& smart_clone_from_another_var(var& r) {
  ...
  if (is_heap()) {
   ...
   else {cout<<"trace:\n\t"<<"in smart_clone_from_another_var: me is in heap but not a object"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<470<<endl;assert(false);};};
  }
  ...
  if (is_heap()) {
   ...
   } else {cout<<"trace:\n\t"<<"in smart_clone_from_another_var: after copy : me is in heap but not a object"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<500<<endl;assert(false);};};
  }
...
# 666 "pypp.hpp"
 ...
 string& refstr() {
  if (is_str()) {
   if (is_stack()) return *data.stack.pstr;
   if (is_heap()) return *data.heap.str.pstr;
  } else {cout<<"trace:\n\t"<<"refstr: not a string : cannot get ref"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<811<<endl;assert(false);};};
 ...
 list tolist() const {
  ...
  } else {cout<<"trace:\n\t"<<"tolist: not a list : cannot get a copy"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<828<<endl;assert(false);};};
  return list();
 }
 ...
 list& reflist() {
  if (is_list()) {
   if (is_stack()) return *data.stack.plist;
   if (is_heap()) return *data.heap.list.plist;
  } else {cout<<"trace:\n\t"<<"reflist: not a list : cannot get ref"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<836<<endl;assert(false);};};
 ...
 tuple totuple() const {
  ...
  } else {cout<<"trace:\n\t"<<"totuple: not a tuple : cannot get copy"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<845<<endl;assert(false);};};
  return tuple();
 }
 ...
 tuple& reftuple() {
  if (is_tuple()) {
   if (is_stack()) return *data.stack.ptuple;
   if (is_heap()) return *data.heap.tuple.ptuple;
  } else {cout<<"trace:\n\t"<<"reftuple: not a tuple : cannot get ref"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<853<<endl;assert(false);};};
 ...
 set toset() const {
  ...
  } else {cout<<"trace:\n\t"<<"toset: not a set : cannot get copy"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<862<<endl;assert(false);};};
  return set();
 }
 ...
 set& refset() {
  if (is_set()) {
   if (is_stack()) return *data.stack.pset;
   if (is_heap()) return *data.heap.set.pset;
  } else {cout<<"trace:\n\t"<<"refset: not a set : cannot get ref"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<870<<endl;assert(false);};};
 ...
 dict todict() const {
  ...
  } else {cout<<"trace:\n\t"<<"todict: not a dict : cannot get copy"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<879<<endl;assert(false);};};
  return dict();
 }
 ...
 dict& refdict() {
  if (is_dict()) {
   if (is_stack()) return *data.stack.pdict;
   if (is_heap()) return *data.heap.dict.pdict;
  } else {cout<<"trace:\n\t"<<"refdict: not a dict : cannot get ref"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<887<<endl;assert(false);};};
...
# 992 "pypp.hpp"
 ...
 friend istream& operator>>(istream& in, var& v) {
  if (v.is_undefined()) {
   ...
   else {cout<<"trace:\n\t"<<"in operator>>: is str but not heap and stack"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<1007<<endl;assert(false);};};
  } else if (v.is_num()) {
   ...
   return in;
  } else {cout<<"trace:\n\t"<<"in operator>>: impossible run here"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<1027<<endl;assert(false);};};
  return in;
 }
...
# 1109 "pypp.hpp"
 ...
 size_t len() {
  ...
  else {cout<<"trace:\n\t"<<"in len: not a sequence"<<endl;{cout<<"die @ "<<"pypp.hpp"<<" line:"<<1118<<endl;assert(false);};};
  return 0;
 }
...
# 16 "pypp-unit-tests.cpp" 2
# 28 "pypp-unit-tests.cpp"
jtest
